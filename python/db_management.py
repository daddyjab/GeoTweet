...
try:
    ...
    # Twitter API
    key_twitter_tweetquestor_consumer_api_key = os.environ[
        'key_twitter_tweetquestor_consumer_api_key']
    key_twitter_tweetquestor_consumer_api_secret_key = os.environ[
    ...
    key_twitter_tweetquestor_access_secret_token = os.environ[
...
try:
    auth = tweepy.OAuthHandler(key_twitter_tweetquestor_consumer_api_key,
                               key_twitter_tweetquestor_consumer_api_secret_key)
    auth.set_access_token(key_twitter_tweetquestor_access_token,
                          key_twitter_tweetquestor_access_secret_token)
...
#
    ...
    try:
        ...
        print(
            f"Twitter API 'trends/{a_type}' - API Calls Remaining: {remaining}")
...
def get_loc_with_trends_available_to_df():
    ...
    try:
        ...
        print(
            f"Tweepy API: Problem getting locations that have trends available information")
        return False
    ...
    trends_avail_df['twitter_type'] = trends_avail_df['placeType'].map(
        lambda x: x['name'])
...
def get_location_info(a_woeid):
    ...
    try:
        ...
        print(
            f"Flickr API: Problem getting location information for WOEID {a_woeid}: ")
        return False
    ...
    if (location_data['stat'] == 'fail'):
        print(
            f"Flickr API: Problem finding location WOEID {a_woeid}: {location_data['message']}")
    ...
    try:
        ...
        print(
            "Error - basic location information not returned for WOEID{a_woeid}: ", sys.exc_info()[0])
...
def update_db_locations_table():
    ...
    loc_info_list = list(
        loc_with_trends_available_df['woeid'].apply(get_location_info))
    ...
    twitter_trend_locations_df = loc_with_trends_available_df.merge(
        loc_info_df, how='inner', on='woeid')
    ...
    twitter_trend_locations_df = twitter_trend_locations_df.where(
        (pd.notnull(twitter_trend_locations_df)), None)
    ...
    for index, row in twitter_trend_locations_df.iterrows():
        ...
        result = db.session.query(Location).filter(
            Location.woeid == row['woeid']).first()
        ...
        if result is None:
            ...
            try:
                db.session.query(Location).filter(
                    Location.woeid == row['woeid']).update(row_dict)
                db.session.commit()
                n_updates += 1
...
def get_trends_for_loc(a_woeid):
    ...
    try:
        ...
        print(
            f"Tweepy API: Problem getting trends information for WOEID {a_woeid}")
        return False
...
def update_db_trends_table():
    ...
    for tw_woeid in loc_list:
        ...
        try:
            ...
            t_info_df.to_sql('trends', con=db.engine,
            ...
            print(
                f">> Error occurred with location {tw_woeid} while attempting to prepare and write trends data")
...
def search_for_tweets(a_search_term):
    ...
    try:
        ...
        tweets = api.search(quote(a_search_term), lang='en',
                            count=tweets_count_limit)

    except:
        # No tweet info available for this search term, return False
        print(
            f"Tweepy API Error: Problem getting tweet information for search term {a_search_term}")
        return False
    ...
    for t in tweets['statuses']:
        ...
        try:
            tweet_info.update({
                ...
                # If True, then this is a Quoted Tweet (i.e., Tweet w/ comments/mods)
                'tweet_is_a_quote_flag': t['is_quote_status'],
            })
        ...
        try:
            tweet_info.update({
                ...
                # No. of Tweets/Retweets issued by this user
                'tweet_user_statuses_count': t['user']['statuses_count'],
                # No. of Tweets this user has liked (in account's lifetime)
                'tweet_user_favourites_count': t['user']['favourites_count'],
                # No. of Followers this account currently has
                'tweet_user_followers_count': t['user']['followers_count'],
                # No. of Users this account is following
                'tweet_user_friends_count': t['user']['friends_count'],
                # No. of Public lists this user is a member of
                'tweet_user_listed_count': t['user']['listed_count']
            })
...
def get_search_terms_from_trends(a_date_range=None):
    ...
    search_term_alt_set = set(
        [f"{y[1:]}" if y[:1] == "#" else f"#{y}" for y in search_term_set])
...
def get_tweet_list():
    ...
    add_search_term_list = list(
        set(trends_search_term_list) - set(tweets_search_term_list))
    print(
        f"Search Terms - Trends: {len(trends_search_term_list)}, Tweets {len(tweets_search_term_list)}, Add: {len(add_search_term_list)}")
    ...
    for s in add_search_term_list:
        ...
        try:
            ...
            print(
                "POSSIBLE RATE LIMITS: search tweets 'remaining' not populated in API results")
            break
        ...
        print(
            f"Search Term '{s}' => Tweet Count: {len(tweets_for_this_search_term)}")
    ...
    print(
        f"OVERALL => Tweet Count: {len(tweet_list)}, API Search Calls: {search_term_count}")
...
def update_db_tweets_table(a_tweet_list):
    ...
    for t in a_tweet_list:
        ...
        result = db.session.query(Tweet).filter(
            Tweet.tweet_id_str == t['tweet_id_str']).first()
        ...
        if result is None:
            ...
            try:
                ...
                n_adds += 1
                print(
                    f">>> ADDED: Record to 'tweets': Search Term '{t['tweet_search_term']}' => Tweet ID: '{t['tweet_id_str']}'")

            except:
                n_error_adds += 1
                print(
                    f">>> ADD: Error while attempting to add record to 'tweets': Search Term '{t['tweet_search_term']}' => Tweet ID: '{t['tweet_id_str']}'")
                db.session.rollback()
            ...
            try:
                db.session.query(Tweet).filter(
                    Tweet.tweet_id_str == t['tweet_id_str']).update(t)
                db.session.commit()
                n_updates += 1
                print(
                    f">>> UPDATED: Record in 'tweets': Search Term '{t['tweet_search_term']}' => Tweet ID: '{t['tweet_id_str']}'")

            except:
                n_error_updates += 1
                print(
                    f">>> UPDATE: Error while attempting to add record to 'tweets': Search Term '{t['tweet_search_term']}' => Tweet ID: '{t['tweet_id_str']}'")
                db.session.rollback()
...
def cleanup_records_before(a_earlierthandatetime_string=None):
    ...
    earlier_than_datetime = None
    n_target_trends = None
    n_target_tweets = None

    if a_earlierthandatetime_string.lower() == 'last update':
        # Argument: "last update":
        # The trends table maintains multiple entries for each twitter_tweet_name (i.e., search terms).
        # Create a target list of records to remove so that only the most recent update remains
        # in the trends table.
        ...
        # Create a target list of records to remove all entries in the trends table
        # that were updated earlier than the a_earlierthandatetime_string

        if a_earlierthandatetime_string.lower() == 'now':
            # Criteria: Current date and time
